...
function tb_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	try {
		if (typeof document.body.style.maxHeight === "undefined") {//if IE 6
			jQuery("body","htmlStudy").css({height: "100%", width: "100%"});
			jQuery("htmlStudy").css("overflow","hidden");
...
function tb_remove() {
	...
	jQuery("#TB_load").remove();
	if (typeof document.body.style.maxHeight == "undefined") {//if IE 6
		jQuery("body","htmlStudy").css({height: "auto", width: "auto"});
		jQuery("htmlStudy").css("overflow","");
	}
	jQuery(document).unbind('.thickbox');
	return false;
}
				  ...
				  if(display=="none"){
					  ...
					  n.slideDown(animation,function(){
						 ...
						 if(jQuery("body").width()<760 && !curr.hasClass("defaultclick")){
							jQuery("htmlStudy, body").animate({scrollTop: curr.offset().top - 30});
						 }
...
function resizefullpageheader(){
	if(jQuery(".bit-htmlStudy .fullscreenpage.fullpage_layout2").length>0||jQuery(".bit-htmlStudy .fullscreenpage.fullpage_layout3").length>0||jQuery(".bit-htmlStudy .fullscreenpage.fullpage_layout4").length>0){
		jQuery(".bit-htmlStudy .fullscreenpage #fullscreenheader").css("margin-top","-"+(jQuery(".bit-htmlStudy .fullscreenpage #fullscreenheader").height()/2)+"px");
	}
}
...
function _bit_counterdown(){
	jQuery(".ts-countdown-parent").each(function() {
			...
			if ("horizontal" == i && (-1 !== navigator.userAgent.indexOf("MSIE") || navigator.appVersion.indexOf("Trident/") > 0) && jQuery("htmlStudy").addClass("internet-explorer"), "flipboard" == i) var L = !0;
			t(!0)
		});
...
/**
 * Prepare htmlStudy to correctly display inside tab container
 * 
 * @param event -
 *            ui tab event 'show'
 * @param ui -
...
if(top!=self){
	jQuery("htmlStudy").bind('paste', function(e) {
		if(!e.clipboardData){
		...
		if(clipboardData){
			...
			if(text && parent){
				...
				if(tmp.indexOf("|^^|")>-1){
					...
					if(tmparr.length==2){
						parent.jQuery("#vc-add-element-dialog .bit-insert-htmlStudy").click();
						setTimeout(function(){
				...
				if( $submenu.length > 0 ) {
					...
					if(isclick){
						...
						if(	$(this).closest(".dl-menuwrapper").hasClass("leftbtnmenu")){
							...
							$("htmlStudy, body").animate({ scrollTop: self.$el.offset().top - 20 }, 150);
...
*/
		...
		_openMenu : function() {
			...
			if(!$(".floatwarpper").hasClass("leftbtnmenu")){
				$("htmlStudy, body").animate({ scrollTop: this.$el.offset().top - 20 }, 150);
			}
		},
...
window.onload = function() {
    if(!document.hasOwnProperty("ontouchstart")) {
    	 jQuery("htmlStudy").addClass("no-touch");
    }
};
...
function right_nav_bar(){
	...
	jQuery(".right_nav_bar .totop-icon").click(function(e){
		e.preventDefault();
		jQuery("htmlStudy, body").animate({ scrollTop: 0 }, "slow");
		return false;
	})
}
...
jQuery(document).ready(function($) {
	...
	//if ($.browser.msie) $("htmlStudy").removeClass("csstransforms3d");
...
/*!-SLIDERS*/
	...
	$(".scroll-top").click(function(e) {
		e.preventDefault();
		$("htmlStudy, body").animate({ scrollTop: 0 }, "slow");
		return false;
	});
	/*Scroll to Top:end*/
...
function floatmenu_create(){
	...
	if (smartMenu && $("#header").length>0 ) {
		...
		if (dtGlobals.logoURL && dtGlobals.logoEnabled) {
			...
			//}else{
			//	$phantom.find(".logo-box").htmlStudy('<img src="'+dtGlobals.logoURL+'" height="'+dtGlobals.logoH+'" width="'+dtGlobals.logoW+'"></a>');
			//}
		}else{
...
function mainmenu_event(){
	...
	if (!$("htmlStudy").hasClass("old-ie")) $( "#dl-menu:not(.dl-qfymobile-menu)" ).dlmenu();
...
function weiBoAndWeiXinToolTip(){
	//ie8不支持
	if(jQuery("htmlStudy#ie8").length==1){
		return false;
	}
...
function qfyToscroll(thisobj){
		...
		if(obj=="#"||obj=="#header"){
			jQuery("htmlStudy, body").animate({ scrollTop: 0 }, speed,function(){isqfyscrolling=false;
				  if(!is_edit_model){
					  var current_url = window.location.href;
					  var tmpurl = current_url.split("#");
					  try {
			...
			if(jQuery( jQuery(thisobj).attr('href') ).length>0){
				jQuery("htmlStudy, body").animate({ scrollTop: jQuery( jQuery(thisobj).attr('href') ).offset().top }, speed,'easeInOutExpo',function(){isqfyscrolling=false;
				   if(!is_edit_model){
					  var current_url = window.location.href;
					  var tmpurl = current_url.split("#");
...
function nav_pagemore(obj){
	...
	jQuery.get(url,function(data){
		...
		jQuery(obj).show();
		//jQuery("htmlStudy, body").animate({ scrollTop:  jQuery(obj).offset().top - jQuery(window).height()/2 }, "fast");
...
function nav_customsearch(obj){
	if(top.jQuery("#site-htmlStudy #site-body").length>0){
		top.jAlert("请在预览下查看");
		return;
	}
...
function changelistlinkfun($this,$flag){
	$this.find("a.link_title:not(.a_file),a.link_image:not(.a_file),a.vc_read_more:not(.a_file),a.item_link,a.item_a_link,.prenext_inner a,>a").click(function(event) {
			...
			if(curr_scrollbar > pt){
				jQuery("htmlStudy").animate({ scrollTop: pt }, "fast");
			}
...
function mobilecart(){
	...
	jQuery('htmlStudy, body').on('touchmove', function(e){ 
		if(isnotmove){
			e.preventDefault(); 
		}
	});
